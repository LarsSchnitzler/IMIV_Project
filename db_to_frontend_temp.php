<?php
//will stay in unixtimecode for all serverside actions. only frontend will convert it to datetime.
...
function getDays() {
    ...
    return array('start' => strtotime($day), 'end' => strtotime($day) - 86400 * $daysBack);
}
...
try {
    ...
    WHERE FROM_UNIXTIME(unixtimestamp_column) <= FROM_UNIXTIME(" + getDays()['start'] + ")  
    AND FROM_UNIXTIME(unixtimestamp_column) >= FROM_UNIXTIME(" + getDays()['end'] + ");");
    $stmt->execute();
